...
async def test_duplicate_feed_rejected_before_writes(duplicate_id, second_status):
    ...
    feed = {
        "vulnerabilities": [
            ...
            {"cveID": duplicate_id, "knownRansomwareCampaignUse": second_status},
        ]
    }
    original = deepcopy(feed)

    with pytest.raises(ValueError, match=f"Duplicate CVE ID in KEV feed: {duplicate_id}"):
...
async def test_unique_feed_keeps_create_update_delete_behavior():
    ...
    created, updated, deleted = await sync_kev_docs(
        {
            "vulnerabilities": [
                {"cveID": "CVE-2026-1001", "knownRansomwareCampaignUse": "Known"},
                {"cveID": "CVE-2026-2001", "knownRansomwareCampaignUse": "Unknown"},
            ]
        }
    )
    ...
    assert await saved_state() == {"CVE-2026-1001": True, "CVE-2026-2001": False}
